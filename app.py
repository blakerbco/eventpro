        ...
        if not rows:
            ...
            for row in rows:
                ...
                if processed >= total_domains:
                    ...
                    # Check if job is actually running by looking at active jobs
                    if job_id in jobs:
                        status = "running"
                ...
                <div class="job-card">
                    ...
                    <div style="font-size:10px;color:#525252;margin-top:8px;">Started: {started_at.strftime('%Y-%m-%d %H:%M:%S') if started_at else 'Unknown'}</div>
                </div>
                """
...
<style>""" + _ADMIN_STYLE + """
  .export-bar { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:20px; }
  .export-bar a, .export-bar button { display:inline-block; padding:8px 16px; border-radius:6px; font-size:12px; font-weight:600; text-decoration:none; font-family:inherit; border:none; cursor:pointer; }
  .btn-gold { background:#eab308; color:#000; }
  ...
  .job-stats { display:grid; grid-template-columns:repeat(auto-fit, minmax(150px, 1fr)); gap:12px; margin-top:12px; }
  ...
  .job-actions a { font-size:11px; padding:6px 12px; }
